...
# 모듈 import
from persona.persona import WVSPersonaGenerator, StatelessPersonaAgent, WVSEthicalQuestions, TOPIC_ALIASES
...
def parse_rating_from_response_advanced(response: str, topic: str, topic_index: int, debug: bool = False) -> int:
    ...
    for alias in aliases:
        # 패턴 리스트 (우선순위 순)
        patterns = [
            # "1. homosexuality: 7" 형태
            rf"^\s*{topic_index}[\.\)]\s*{re.escape(alias)}\s*[:=\-–]\s*(\d+)",
            # "homosexuality: 7" (번호 없이)
            rf"^\s*{re.escape(alias)}\s*[:=\-–]\s*(\d+)",
            # 중간에 텍스트가 있는 경우
            rf"^\s*{topic_index}[\.\)]\s*{re.escape(alias)}.*?[:=\-–]\s*(\d+)",
            # 줄 어디든 별칭 + 숫자
            rf"\b{re.escape(alias)}\b.*?[:=\-–]\s*(\d+)",
        ]
        ...
        for line in lines:
            ...
            for pattern in patterns:
                ...
                if match:
                    ...
                    if 1 <= rating <= 10:
                        if debug:
                            print(f"✓ Matched '{topic}' (alias: {alias}) in line: '{line_clean[:80]}'")
                        return rating
    
    # 전체 응답에서 검색
    for alias in aliases:
        global_pattern = rf"\b{re.escape(alias)}\b\s*[:=\-–]\s*(\d+)"
        global_match = re.search(global_pattern, response, re.IGNORECASE)
        if global_match:
            rating = int(global_match.group(1))
            if 1 <= rating <= 10:
                if debug:
                    print(f"✓ Matched '{topic}' (alias: {alias}) globally")
                return rating
    
    # 번호 기반 검색 (주제명 없이 번호만 있는 경우)
    ...
    for line in lines:
        ...
        if match:
            ...
            if 1 <= rating <= 10:
                if debug:
                    print(f"✓ Matched '{topic}' by index {topic_index} in line: '{line.strip()}'")
                return rating
